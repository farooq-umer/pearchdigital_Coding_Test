...
class JiraClient
{
    ...
    public function __construct(ConfigurationInterface $configuration = null, Logger $logger = null, $path = './')
    {
        ...
        if ($logger) {
            ...
            $this->log->pushHandler(new StreamHandler(
                ...
                //added our logFile path to capture all logs in our logFile
                LOCAL_LOGS_DIR.'/'.JIRA_LOG_FILENAME,
                $this->convertLogLevel($configuration->getJiraLogLevel())
            ));

            //$this->log->log($this->convertLogLevel($configuration->getJiraLogLevel()), 'My logger is now ready');
        }
    ...
    private function createDownloadHandle($url, $download_dir)
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_URL, $url);
    ...
    //Asad
    public function download($file_name, $fileUrl, $attachment_directory)
    {
        $saveTo = $attachment_directory.'/'.$file_name;
        $fp = fopen($saveTo, 'w+');
        //If $fp is FALSE, something went wrong.
        if($fp === false){
            throw new JiraException('Could not open: ' . $saveTo);
        }
        ...
        if(curl_errno($ch)){
            throw new JiraException(curl_error($ch));
        }
