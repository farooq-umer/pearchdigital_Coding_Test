...
class IssueService extends \JiraRestApi\JiraClient
{
    ...
    public function downloadAttachments($file_name, $fileUrl, $attachment_directory)
    {
        $statusCode = 200;
        /*echo 'file_name ' . $file_name .'<br />';
        echo 'fileUrl ' . $fileUrl .'<br />';
        echo 'attachment_directory ' . $attachment_directory . '<br />';*/
        $statusCode = $this->download($file_name, $fileUrl, $attachment_directory);
        //echo $statusCode . '<br />';
        //echo $file_name . '<br />';
        return $statusCode;
    }
    ...
    public function getAttachments($issueIdOrKey)
    {
        ...
        $this->log->addInfo("getAttachments=\n");

        $issueObject =  new Issue();

        $ret = $this->exec($this->uri."/".$issueIdOrKey);

        $this->log->addDebug('get comment attachments='.var_export($ret, true));

        $issue = $this->json_mapper->map(
            json_decode($ret), $issueObject
        );

        if(count($issue->fields->attachment)){
     ...
     */
    public function getCreateMeta($paramArray = [], $expand = true, $assoc = false)
    {
        ...
        return json_decode($ret, $assoc);
    }

    //Asad
    public function getJiraCustomers($projectKeys = ''){

        if($projectKeys == ''){
            throw new JiraException('Project code is not provided.');
        }
        ...
        $issue_meta = $this->getCreateMeta($param, true); //object(stdClass
        $meta_porjects = $issue_meta->projects; //array (size=1)
        if(count($meta_porjects) > 0)
        {

            /*$issuetypes = $meta_porjects[0]->issuetypes; //array (size=6)
            $fields = $issuetypes[0]->fields; // object(stdClass)[24]
            return $fields;*/
            $issuetypes = $meta_porjects[0]->issuetypes;
            $fields = $issuetypes[0]->fields;
            $customfield = $fields->customfield_10044; // object(stdClass)[40]
            $allowed_values = $customfield->allowedValues;

            return $allowed_values;

        }else {
            throw new JiraException('No customers are found on Jira. Project Key : '.$projectKeys);
        }

    }

    public function getJiraCustomFields($projectKeys = '')
    {
        if($projectKeys == '')
        {
            throw new JiraException('Project code is not provided.');
        ...
        $issue_meta = $this->getCreateMeta($param, true, true); //object(stdClass
        $meta_porjects = $issue_meta['projects']; //array (size=1)

        $issuetypes = $meta_porjects[0]['issuetypes']; //array (size=6)
        $fields = $issuetypes[0]['fields']; // object(stdClass)[24]
        $cfs = array_filter( $fields, function ($key) {
            $pos = strpos($key, 'customfield_');
            return $pos !== false;
    ...
    //asad 29/04/2018
    public function getHistory($issueIdOrKey, $paramArray = [])
    {
        $this->log->addInfo("getHistory=\n");

        $issueObject = new Issue();

        $queryParam = '?'.http_build_query($paramArray);

        $ret = $this->exec($this->uri."/$issueIdOrKey".$queryParam);

        $this->log->addDebug('get History result='.var_export($ret, true));
        $history = $this->json_mapper->map(
            json_decode($ret), $issueObject
        );
    ...
    public function updateComment($issueIdOrKey, $commentId, $param = [])
    {
        $this->log->addInfo("updateComment=\n");

        $data = json_encode($param);

        $ret = $this->exec($this->uri."/$issueIdOrKey/comment/".$commentId, $data, 'PUT');

        // commented out 04-7-18 due to an error Undefined variable: assigneeName
        //$this->log->addInfo('change assignee of '.$issueIdOrKey.' to '.$assigneeName.' result='.var_export($ret, true));
